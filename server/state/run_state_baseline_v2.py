...
def parse_args():
    ...
    p.add_argument("--roi-delta-weight",type=float,default=0.0)
    p.add_argument("--screw-detect-weight",type=float,default=0.0)
...
def build_ordered_evidence(df, tool_threshold, index_weight, roi_delta_weight, screw_detect_weight):
    df=df.copy()
    for k in ["a","b","c","d"]:
        tool=to_float(df,f"tool_near_{k}_smooth",0.0); idx=to_float(df,f"index_near_{k}_smooth",0.0)
        delta=to_float(df,f"screw_{k}_done_score_delta_smooth",0.0)
        screw=to_float(df,f"tip_screw_score_{k}_smooth",0.0)
        df[f"event_{k}_score"]=(tool+index_weight*idx+roi_delta_weight*delta+screw_detect_weight*screw).clip(0,1)
    seen={k:[] for k in ["a","b","c","d"]}; progress={k:0.0 for k in ["a","b","c","d"]}
    order=["a","b","c","d"]
    for _,r in df.iterrows():
        progress["a"]=max(progress["a"],float(r["event_a_score"]))
        progress["b"]=max(progress["b"],min(progress["a"],float(r["event_b_score"])))
        progress["c"]=max(progress["c"],min(progress["b"],float(r["event_c_score"])))
        progress["d"]=max(progress["d"],min(progress["c"],float(r["event_d_score"])))
        for k in order: seen[k].append(progress[k])
    for k in order: df[f"ordered_seen_{k}"]=seen[k]
    return df

def build_scores(df, roi_weight, roi_delta_weight, screw_detect_weight):
    ...
    delta={k:to_float(df,f"screw_{k}_done_score_delta_smooth",0.0).to_numpy() for k in ["a","b","c","d"]}
    screw={k:to_float(df,f"tip_screw_score_{k}_smooth",0.0).to_numpy() for k in ["a","b","c","d"]}
    box_delta=to_float(df,"box_present_score_delta_smooth",0.0).to_numpy()
    lid_delta=to_float(df,"lid_closed_score_delta_smooth",0.0).to_numpy()
    ...
    for i in range(n):
        ...
        scores[i,1]=floor+0.35*box[i]*(1-lid[i])+roi_delta_weight*box_delta[i]*q[i]
        scores[i,2]=floor+0.45*lid[i]*(1-seen['a'][i])+roi_delta_weight*lid_delta[i]*q[i]
        scores[i,3]=floor+0.80*seen['a'][i]*(1-seen['b'][i])+roi_weight*roi['a'][i]*q[i]+roi_delta_weight*delta['a'][i]*q[i]+screw_detect_weight*screw['a'][i]*q[i]
        scores[i,4]=floor+0.80*seen['b'][i]*(1-seen['c'][i])+roi_weight*roi['b'][i]*q[i]+roi_delta_weight*delta['b'][i]*q[i]+screw_detect_weight*screw['b'][i]*q[i]
        scores[i,5]=floor+0.80*seen['c'][i]*(1-seen['d'][i])+roi_weight*roi['c'][i]*q[i]+roi_delta_weight*delta['c'][i]*q[i]+screw_detect_weight*screw['c'][i]*q[i]
        scores[i,6]=floor+0.80*seen['d'][i]+roi_weight*roi['d'][i]*q[i]+roi_delta_weight*delta['d'][i]*q[i]+screw_detect_weight*screw['d'][i]*q[i]
...
def predict_fsm(df,min_dwell,tool_threshold,roi_delta_weight):
    cur=0; dwell=0; preds=[]
    ...
    box_sig=np.clip(box+roi_delta_weight*to_float(df,"box_present_score_delta_smooth",0.0).to_numpy(),0,1)
    lid_sig=np.clip(lid+roi_delta_weight*to_float(df,"lid_closed_score_delta_smooth",0.0).to_numpy(),0,1)
    ev={k:to_float(df,f"event_{k}_score",0.0).to_numpy() for k in ["a","b","c","d"]}
    for i in range(len(df)):
        if dwell>=min_dwell:
            if cur==0 and box_sig[i]>=0.50: cur=1; dwell=0
            elif cur==1 and lid_sig[i]>=0.50: cur=2; dwell=0
...
def main():
    ...
    df=build_ordered_evidence(df,args.tool_threshold,args.index_weight,args.roi_delta_weight,args.screw_detect_weight); scores=build_scores(df,args.roi_weight,args.roi_delta_weight,args.screw_detect_weight)
    pred_framewise=predict_framewise(scores); pred_fsm=predict_fsm(df,args.min_dwell,args.tool_threshold,args.roi_delta_weight); pred_hmm=viterbi(scores,args.self_prob,args.next_prob)
